...
class CRMEntity
{
	...
	public function initialize()
	{
		...
		$userSpecificTableIgnoredModules = ['SMSNotifier', 'PBXManager', 'ModComments'];
	...
	public function insertIntoCrmEntity($module, $fileid = '')
	{
		...
		switch ($module) {
			case 'HelpDesk': $entityFieldNames = ['ticket_title'];

				break;
			case 'Documents': $entityFieldNames = ['notes_title'];
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if (in_array($datatype, ['N', 'NN'])) {
						$fldvalue = CurrencyField::convertToDBFormat($this->column_fields[$fieldname], $current_user, true);
					}
				} elseif ($uitype == 1 && in_array($datatype, ['N', 'NN']) && in_array($fieldname, ['qty_per_unit', 'qtyinstock', 'salescommission', 'exciseduty'])) {
					$fldvalue = CurrencyField::convertToDBFormat($this->column_fields[$fieldname], $current_user, true);
				} elseif ($uitype == 26) {
					...
					if (($insertion_mode == 'edit' && $skipUpdateForField == false) || $_REQUEST['imgDeleted']) {
						...
						$imageNamesRes = $adb->pquery($getImageNamesSql, [$this->id]);
...
//Added by Richie
	...
	public function unlinkRelationship($id, $return_module, $return_id)
	{
		...
		if ($return_module == 'Documents') {
			...
			$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM vtiger_field WHERE fieldid IN (SELECT fieldid FROM vtiger_fieldmodulerel WHERE module=? AND relmodule=?)', [$currentModule, $return_module]);
			$numOfFields = $this->db->num_rows($fieldRes);
			for ($i = 0; $i < $numOfFields; $i++) {
				$tabId = $this->db->query_result($fieldRes, $i, 'tabid');
				$tableName = $this->db->query_result($fieldRes, $i, 'tablename');
	...
	public function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '')
	{
		...
		if ($mode == 'configure' && $req_no != '') {
			$check = $adb->pquery('select cur_id from vtiger_modentity_num where semodule=? and prefix = ?', [$module, $req_str]);
			if ($adb->num_rows($check) == 0) {
				...
				$adb->pquery('INSERT into vtiger_modentity_num values(?,?,?,?,?,?)', [$numid, $module, $req_str, $req_no, $req_no, 1]);
				...
				if ($req_no < $num_check) {
					...
					$adb->pquery('UPDATE vtiger_modentity_num SET cur_id=?, active = 1 where prefix=? and semodule=?', [$req_no, $req_str, $module]);
	...
	public function checkModuleSeqNumber($table, $column, $no)
	{
		...
		$result = $adb->pquery('select ' . $adb->sql_escape_string($column) .
				...
				' where ' . $adb->sql_escape_string($column) . ' = ?', [$no]);
	...
	public function updateMissingSeqNumber($module)
	{
		...
		$tabid = getTabid($module);
		$fieldinfo = $adb->pquery('SELECT tablename, columnname FROM vtiger_field WHERE tabid = ? AND uitype = 4', [$tabid]);
	...
	public function getSequnceNumber($curid)
	{
		$strip = strlen($curid) - strlen($curid + 1);
		if ($strip < 0) {
			$strip = 0;
	 ...
	 * @param mixed $module
	 * @param mixed $crmid
	 * @param mixed $with_module
	 * @param mixed $with_crmid
	 */
		...
		foreach ($with_crmid as $relcrmid) {
			if ($with_module == 'Documents') {
				...
				$checkpresence = $adb->pquery('SELECT crmid FROM vtiger_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?', [$crmid, $module, $relcrmid, $with_module]);
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence)) {
					continue;
				}
	 ...
	 */
	public function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $relationId)
	{
		...
		$result = $this->db->pquery($relationFieldSql, [$relationId]);
		...
		if (empty($relationFieldId)) {
			$dependentFieldSql = $this->db->pquery("SELECT tabid, fieldname, columnname,tablename FROM vtiger_field WHERE uitype='10' AND" .
					' fieldid IN (SELECT fieldid FROM vtiger_fieldmodulerel WHERE relmodule=? AND module=?)', [$currentModule, $related_module]);
		} else {
			$dependentFieldSql = $this->db->pquery("SELECT tabid, fieldname, columnname,tablename FROM vtiger_field WHERE uitype='10' AND" .
					' fieldid IN (SELECT relationfieldid FROM vtiger_relatedlists WHERE relation_id=?)', [$relationId]);
		}
	...
	public function transferRelatedRecords($module, $transferEntityIds, $entityId)
	{
		...
		foreach ($transferEntityIds as $transferId) {
			...
			$relatedRecords = $adb->pquery('SELECT relcrmid, relmodule FROM vtiger_crmentityrel WHERE crmid=? AND module=?' .
					' AND relcrmid NOT IN (SELECT relcrmid FROM vtiger_crmentityrel WHERE crmid=? AND module=?)', [$transferId, $module, $entityId, $module]);
			...
			$parentRecords = $adb->pquery('SELECT crmid, module FROM vtiger_crmentityrel WHERE relcrmid=? AND relmodule=?' .
					' AND crmid NOT IN (SELECT crmid FROM vtiger_crmentityrel WHERE relcrmid=? AND relmodule=?)', [$transferId, $module, $entityId, $module]);
	...
	public function generateReportsSecQuery($module, $secmodule, $queryPlanner)
	{
		...
		$fields_query = $adb->pquery('SELECT vtiger_field.fieldname,vtiger_field.tablename,vtiger_field.fieldid from vtiger_field INNER JOIN vtiger_tab on vtiger_tab.name = ? WHERE vtiger_tab.tabid=vtiger_field.tabid AND vtiger_field.uitype IN (10) and vtiger_field.presence in (0,2)', [$secmodule]);
	...
	public function getReportsUiType10Query($module, $queryPlanner)
	{
		$adb = PearDatabase::getInstance();
		$relquery = '';
		$matrix = $queryPlanner->newDependencyMatrix();
	...
	public function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner)
	{
		...
		$secQueryTempTableQuery = $queryPlanner->registerTempTable($secQuery, [$column_name, $fields[1], $prifieldname], $secmodule);
	...
	public function getJoinClause($tableName)
	{
		if (strripos($tableName, 'rel') === (strlen($tableName) - 3)) {
			return 'LEFT JOIN';
		} elseif (Vtiger_Functions::isUserSpecificFieldTable($tableName, $this->moduleName)) {
			return 'LEFT JOIN';
		} else {
			return 'INNER JOIN';
		}
	 ...
	 * @param <type> $userGroups
	 */
	public function getNonAdminUserGroupAccessQuery($userGroups)
	{
		return ' UNION (SELECT groupid FROM vtiger_groups WHERE groupid IN (' . implode(',', $userGroups) . '))';
	}
	...
	public function get_comments($relatedRecordId = false)
	{
		...
		} elseif (is_array($this->column_fields)) {
...
class TrackableObject implements ArrayAccess, IteratorAggregate
{
	...
	public function pauseTracking()
	{
		$this->tracking = false;
	}

	public function resumeTracking()
	{
	...
	public function getColumnFieldNames()
	{
