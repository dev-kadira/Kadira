...
class Google_Calendar_Connector extends WSAPP_TargetConnector
{
	...
	public function googleFormat($date)
	{
		...
		return $datTime->format('Y-m-d\TH:i:s\Z');
	}
	...
	public function getCalendar($SyncState, $user = false)
	{
		...
		foreach ($calendarRecords as $i => $calendar) {
			$recordModel = Google_Calendar_Model::getInstanceFromValues(['entity' => $calendar]);
			$deleted = false;
	...
	public function transformToTargetRecord($vtEvents, $user)
	{
		...
		foreach ($vtEvents as $vtEvent) {
			...
			$recordModel = Google_Calendar_Model::getInstanceFromValues(['entity' => $newEvent]);
	...
	protected function putGoogleEventCalendarMap($event_ids, $calendar_id, $user)
	{
		if (is_array($event_ids) && count($event_ids)) {
			...
			$query = 'DELETE FROM vtiger_google_event_calendar_mapping WHERE event_id IN (' . generateQuestionMarks($event_ids) . ')';
			$db->pquery($query, $event_ids);
