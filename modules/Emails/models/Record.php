...
class Emails_Record_Model extends Vtiger_Record_Model
{
	...
	public function getFromEmailAddress()
	{
		...
		$fromEmail = false;
		$result = $db->pquery('SELECT from_email_field FROM vtiger_systems WHERE server_type=?', ['email']);
	...
	public function getAttachmentDetails()
	{
		...
		$attachmentRes = $db->pquery('SELECT * FROM vtiger_attachments
						...
						WHERE vtiger_seattachmentsrel.crmid = ?', [$this->getId()]);
	...
	public function getRelatedDocuments()
	{
		...
		$documentRes = $db->pquery('SELECT * FROM vtiger_senotesrel
						...
						WHERE vtiger_crmentity.deleted = 0', [$this->getId()]);
	...
	public function saveDocumentDetails()
	{
		...
		for ($i = 0; $i < $count; $i++) {
			$db->pquery('INSERT INTO vtiger_senotesrel(crmid, notesid) VALUES(?, ?)', [$record, $documentIds[$i]]);
		}
	}
	 ...
	 */
	public function deleteDocumentLink($idList = [])
	{
		$db = PearDatabase::getInstance();
	 ...
	 */
	public function deleteAttachment($emailAttachmentDetails = [])
	{
		$db = PearDatabase::getInstance();
	...
	public function getTrackImageDetails($crmId, $emailTrack = true)
	{
		...
		if ($emailTrack) {
			...
			return Vtiger_Functions::getTrackImageContent($emailId, $crmId);
		} else {
			return null;
		}
	}
	...
	public function getClickCountValue($parentId)
	{
		...
		$result = $db->pquery('SELECT click_count FROM vtiger_email_track WHERE crmid = ? AND mailid = ?', [$parentId, $this->getId()]);
	...
	public function getAccessCountValue($parentId)
	{
		...
		$result = $db->pquery('SELECT access_count FROM vtiger_email_track WHERE crmid = ? AND mailid = ?', [$parentId, $this->getId()]);
	...
	public static function getTrackingInfo($emailIds, $parentId)
	{
		...
		if ($numRows > 0) {
			for ($i = 0;$i < $numRows;$i++) {
				...
				$trackingInfo[$row['mailid']] = ['access_count' => $row['access_count'], 'click_count' => $row['click_count']];
			}
		}
	...
	public function getEmailFlag()
	{
		if (! array_key_exists('email_flag', $this->getData())) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT email_flag FROM vtiger_emaildetails WHERE emailid = ?', [$this->getId()]);
			if ($db->num_rows($result) > 0) {
	...
	{
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT from_email,idlists FROM vtiger_emaildetails WHERE emailid = ?', [$this->getId()]);
		if ($db->num_rows($result) > 0) {
	 ...
	 * @param mixed $fieldId
	 */
	public function recieveEmailLookup($fieldId, $values)
	{
		$db = PearDatabase::getInstance();
	 ...
	 * @param type $fieldid
	 */
	public function deleteEmailLookup($crmid, $fieldid = false)
	{
		$db = PearDatabase::getInstance();
		if ($fieldid) {
	...
	public function updateEmailFlag()
	{
		...
		$db->pquery($query, [$this->get('id')]);
	}

	public function replaceMergeTags($id)
	{
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
	 ...
	 */
	public function isEmailOpenedRecently($parentId)
	{
		$db = PearDatabase::getInstance();
		$lastOpenTime = date('Y-m-d H:i:s', strtotime('-1 hours'));

		$result = $db->pquery('SELECT 1 FROM vtiger_email_access WHERE crmid = ? AND mailid = ? AND accesstime > ?', [$parentId, $this->getId(), $lastOpenTime]);
		if ($db->num_rows($result)) {
			return true;
		}
