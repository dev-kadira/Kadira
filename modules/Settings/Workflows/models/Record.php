...
class Settings_Workflows_Record_Model extends Settings_Vtiger_Record_Model
{
	...
	public function getEntityMethods()
	{
		...
		return $emm->methodsForModule($this->get('module_name'));
	}
	...
	public function executionConditionAsLabel($executionCondition = null)
	{
		...
		$arr = ['ON_FIRST_SAVE', 'ONCE', 'ON_EVERY_SAVE', 'ON_MODIFY', '', 'ON_SCHEDULE', 'MANUAL'];

		return $arr[$executionCondition - 1];
	}

	public function getV7executionConditionAsLabel($executionCondition = null, $module_name)
	{
		if ($executionCondition == null) {
			$executionCondition = $this->get('execution_condition');
		}
		$module = 'Settings:Workflows';
		$arr = [vtranslate($module_name, $module_name) . ' ' . vtranslate('LBL_CREATION', $module),
	...
	public function getDependentModules()
	{
		...
		$filterModules = ['Invoice', 'Quotes', 'SalesOrder', 'PurchaseOrder', 'Emails', 'Calendar', 'Events'];
	...
	public static function getUpdateFieldTaskIdsForModule($moduleName, $fieldName)
	{
		...
		$sql = 'SELECT * FROM com_vtiger_workflows
			INNER JOIN com_vtiger_workflowtasks ON com_vtiger_workflows.workflow_id = com_vtiger_workflowtasks.workflow_id
 			WHERE module_name = ? AND task LIKE ? AND task LIKE ? ';

		$result = $db->pquery($sql, [$moduleName, '%VTUpdateFieldsTask%', '%' . $fieldName . '%']);
	...
	public function getConditonDisplayValue()
	{
		...
		$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
		$wfCond = json_decode(strval($test), true);
		...
		if (is_array($wfCond)) {
			for ($k = 0; $k < (count($wfCond)); ++$k) {
				...
				if ($value) {
					...
					if ($fieldModel && (in_array($fieldDataType, ['picklist', 'multipicklist']))) {
