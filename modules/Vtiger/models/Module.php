...
class Vtiger_Module_Model extends Vtiger_Module
{
	...
	public function getNameFields()
	{
		...
		if ($nameFieldObject && $nameFieldObject->fieldname) {
			...
			switch ($moduleName) {
				case 'HelpDesk': 
					$this->nameFields = ['ticket_title']; $fieldNames = 'ticket_title';
					break;
				case 'Documents': 
					$this->nameFields = ['notes_title']; $fieldNames = 'notes_title';
					break;
			}
	...
	public function getRecentRecords($limit = 10)
	{
		...
		$result = $db->pquery($query, [$this->getName(), $currentUserModel->id, $limit]);
	...
	public function getAutoFillModule($moduleName)
	{
		$autoFillModules = [];
		switch ($moduleName) {
			case 'Contacts': $autoFillModules[] = 'Accounts';

break;
			case 'Project': $autoFillModules = ['Contacts', 'Accounts'];
...
break;
	...
	{
		return [1 => 'LBL_PREFER_LATEST_RECORD',
