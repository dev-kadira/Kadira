...
class Vtiger_Module_Model extends Vtiger_Module
{
	...
	public function isCommentEnabled()
	{
		...
		if ($commentsModuleModel && $commentsModuleModel->isActive()) {
			$relatedToFieldResult = $db->pquery(
				'SELECT fieldid FROM vtiger_field WHERE fieldname = ? AND tabid = ?',
				['related_to', $commentsModuleModel->getId()]
			);
			$fieldId = $db->query_result($relatedToFieldResult, 0, 'fieldid');
			if (! empty($fieldId)) {
				$relatedModuleResult = $db->pquery('SELECT relmodule FROM vtiger_fieldmodulerel WHERE fieldid = ?', [$fieldId]);
	...
	public function getAllFilterCvidForModule()
	{
		...
		$result = $db->pquery(
			"SELECT cvid FROM vtiger_customview WHERE viewname = 'All' AND entitytype = ?",
			[$this->getName()]
		);
	...
	public function getNameFields()
	{
		...
		if ($nameFieldObject && $nameFieldObject->fieldname) {
			...
			switch ($moduleName) {
				case 'HelpDesk': $this->nameFields = ['ticket_title']; $fieldNames = 'ticket_title';

break;
				case 'Documents': $this->nameFields = ['notes_title']; $fieldNames = 'notes_title';
...
break;
	...
	public function getRecentRecords($limit = 10)
	{
		...
		$query = 'SELECT * FROM vtiger_crmentity ' . $nonAdminQuery . ' WHERE setype=? AND ' . $deletedCondition .
			' AND modifiedby = ? ORDER BY modifiedtime DESC LIMIT ?';
	 ...
	 */
	public static function getAll($presence = [], $restrictedModulesList = [], $sequenced = false)
	{
		$db = PearDatabase::getInstance();
	...
	public static function getEntityModules()
	{
		...
		if (! $moduleModels) {
			...
			$restrictedModules = ['Webmails', 'Emails', 'Integration', 'Dashboard'];
			foreach ($moduleModels as $key => $moduleModel) {
	...
	public function getExportQuery($focus, $where)
	{
		...
		return $focus->create_export_query($where);
	}
	...
	public function getOwnerWhereConditionForDashBoards($owner)
	{
		...
		if (! empty($owner) && $currentUserModel->isAdminUser()) {//If admin user, then allow users data
			...
			$params[] = $owner;
		} elseif (! empty($owner)) {//If not admin user, then check sharing access for that module
	...
	public function getCreateEventRecordUrl()
	{
		...
		$createViewUrl = $relatedModel->getCreateEventRecordUrl() .
	...
	public function getCreateTaskRecordUrl()
	{
		...
		$createViewUrl = $relatedModel->getCreateTaskRecordUrl() .
			'&returnmode=showRelatedList&returntab_label=' . $relationModel->get('label') .
			'&returnrecord=' . $parentRecordModule->getId() . '&returnmodule=' . $parentModule->get('name') .
			'&returnview=Detail&returnrelatedModuleName=Calendar' .
			'&returnrelationId=' . $relationModel->getId();
	...
	public function getSearchRecordsQuery($searchValue, $searchFields, $parentId = false, $parentModule = false)
	{
		...
		return $db->convert2Sql('SELECT ' . implode(',', $searchFields) . ' FROM vtiger_crmentity WHERE label LIKE ? AND vtiger_crmentity.deleted = 0', ["%${searchValue}%"]);
	}
	...
	{
		$modulesList = ['Faq', 'PriceBook', 'Vendors', 'Users'];
	...
	public function getRelatedModuleRecordIds(Vtiger_Request $request, $recordIds = [], $nonAdminCheck = false)
	{
		$db = PearDatabase::getInstance();
		...
		for ($i = 0 ;$i < $num_rows; $i++) {
			...
			$relationFieldMapping[] = ['relatedModuleName'=>$relatedModuleModel->getName(), 'relationfieldid'=>$relationfieldid];
		}
			...
			foreach ($relationFieldMapping as $mappingDetails) {
				...
				if (empty($relationFieldId)) {
					...
					if ($tablename == 'vtiger_crmentityrel') {
						...
						if (in_array($tablename, ['vtiger_senotesrel'])) {
							$sql .= ' LEFT JOIN vtiger_activity ON vtiger_activity.activityid = vtiger_crmentity.crmid ';
						}
						$sql .= " INNER JOIN ${tablename} ON ${tablename}.${tabIndex} = vtiger_crmentity.crmid
	...
	public function getAutoFillModule($moduleName)
	{
		...
		switch ($moduleName) {
			case 'Contacts':
				$autoFillModules[] = 'Accounts';

				break;
			case 'Project':
				$autoFillModules = ['Contacts', 'Accounts'];
	...
	{
		return ['Import', 'Export', 'Merge', 'DuplicatesHandling'];
	}
	 ...
	 */
	public function getUtilityActions()
	{
		$utilityActions = [];
	 ...
	 */
	public function getExtensionLaunchUrl()
	{
	...
	{
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT appname,visible FROM vtiger_app2tab WHERE tabid = ?', [$this->getId()]);
