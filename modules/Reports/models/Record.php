...
class Reports_Record_Model extends Vtiger_Record_Model
{
	...
	public function getSelectedFields()
	{
		...
		for ($i = 0; $i < $db->num_rows($result); $i++) {
			...
			} elseif ($primaryModule == 'PriceBooks' && $fieldName == 'listprice' && in_array($module, ['Products', 'Services'])) {
				// to support pricebooks listprice in reports
				$selectedColumns[$module . '_' . $translatedFieldLabel] = $column;
			} elseif (CheckFieldPermission($fieldName, $module) == 'true') {
	...
	public function getSelectedStandardFilter()
	{
		...
		$result = $db->pquery(
			'SELECT * FROM vtiger_reportdatefilter WHERE datefilterid = ? AND startdate != ? AND enddate != ?',
			[$this->getId(), '0000-00-00', '0000-00-00']
		);
		$standardFieldInfo = [];
	...
	public function save()
	{
		...
		if (empty($reportId)) {
			...
			$db->pquery(
				...
				[$reportId, $this->getPrimaryModule(), $secondaryModule]
			);
			...
			$db->pquery(
				'UPDATE vtiger_reportmodules SET primarymodule = ?,secondarymodules = ? WHERE reportmodulesid = ?',
				[$this->getPrimaryModule(), $this->getSecondaryModules(), $reportId]
			);

			$db->pquery('UPDATE vtiger_report SET reportname = ?, description = ?, reporttype = ?, folderid = ?,sharingtype = ? WHERE
				reportid = ?', [decode_html($this->get('reportname')), decode_html($this->get('description')),
		...
		if (! empty($sortFields)) {
			...
			foreach ($sortFields as $fieldInfo) {
				$db->pquery(
					...
					[$i, $this->getId(), $fieldInfo[0], $fieldInfo[1]]
				);
	...
	public function saveCalculationFields()
	{
		...
		for ($i = 0; $i < count($calculationFields); $i++) {
			$query = 'INSERT INTO vtiger_reportsummary (reportsummaryid, summarytype, columnname) VALUES (?,?,?)';
			$db->pquery($query, [$this->getId(), $i, $calculationFields[$i]]);
		}
	}
	...
	public function saveStandardFilter()
	{
		...
		if (! empty($standardFilter)) {
			$query = 'INSERT INTO vtiger_reportdatefilter (datefilterid, datecolumnname, datefilter, startdate, enddate) VALUES (?,?,?,?,?)';
			$db->pquery($query, [$this->getId(), $standardFilter['field'], $standardFilter['type'],
	...
	public function saveSharingInformation()
	{
		...
		for ($i = 0; $i < count($sharingInfo); $i++) {
			$db->pquery(
				...
				[$reportId, $sharingInfo[$i]['id'], $sharingInfo[$i]['type']]
			);
		}

		//On every report save delete information from below tables and insert new to avoid
		// confusion in updating
		$db->pquery('DELETE FROM vtiger_report_shareusers WHERE reportid=?', [$reportId]);
		$db->pquery('DELETE FROM vtiger_report_sharegroups WHERE reportid=?', [$reportId]);
		$db->pquery('DELETE FROM vtiger_report_sharerole WHERE reportid=?', [$reportId]);
		$db->pquery('DELETE FROM vtiger_report_sharers WHERE reportid=?', [$reportId]);
			...
			for ($i = 0; $i < $noOfMembers; ++$i) {
				...
				if ($idComponents && count($idComponents) == 2) {
					...
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_USERS) {
						$db->pquery('INSERT INTO vtiger_report_shareusers(userid, reportid) VALUES (?,?)', [$memberId, $reportId]);
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_GROUPS) {
						$db->pquery('INSERT INTO vtiger_report_sharegroups(groupid, reportid) VALUES (?,?)', [$memberId, $reportId]);
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLES) {
						$db->pquery('INSERT INTO vtiger_report_sharerole(roleid, reportid) VALUES (?,?)', [$memberId, $reportId]);
					}
					if ($memberType == Settings_Groups_Member_Model::MEMBER_TYPE_ROLE_AND_SUBORDINATES) {
						$db->pquery('INSERT INTO vtiger_report_sharers(rsid, reportid) VALUES (?,?)', [$memberId, $reportId]);
					}
				}
			}
		}
			...
			for ($i = 0 ;$i < count($selectedFields);$i++) {
				if (! empty($selectedFields[$i])) {
					$db->pquery(
						...
						[$this->getId(), $i, decode_html($selectedFields[$i])]
					);
				}
			}
		}
		...
		if (! empty($advancedFilter)) {
			...
			foreach ($advancedFilter as $groupIndex => $groupInfo) {
				...
				$db->pquery(
					...
					[$groupIndex, $reportId, $groupCondition, $groupConditionExpression]
				);
			}
		}
	}
	...
	public function getReportSQL($advancedFilterSQL = false, $format = false)
	{
		...
		return $reportRun->sGetSQLforReport($this->getId(), $advancedFilterSQL, $format);
	}
	...
	public function generateCountQuery($query)
	{
		...
		return 'SELECT count(*) AS count FROM ' . $fromAndWhereQuery[0];
	}
	/**
	...
	public function getReportData($pagingModel = false, $filterQuery = false)
	{
		...
		return $reportRun->GenerateReport('PDF', $filterQuery, true, $pagingModel->getStartIndex(), $pagingModel->getPageLimit());
	}
	...
	public function getReportCalulationData($filterQuery = false)
	{
		...
		return $reportRun->GenerateReport('TOTALXLS', $filterQuery, true);
	}
	/**
	...
	public function transformStandardFilter()
	{
		...
		if (! empty($standardFilter)) {
			...
			$standardReports = ['Last Month Activities', 'This Month Activities'];
	...
	public function getAdvancedFilterSQL()
	{
		...
		return $this->reportRun->RunTimeAdvFilter($advancedFilterCriteria, $advancedFilterCriteriaGroup);
	}
	...
	public function isPinnedToDashboard()
	{
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$result = $db->pquery('SELECT 1 FROM vtiger_module_dashboard_widgets WHERE reportid = ? AND userid = ?', [$this->getId(), $currentUser->getId()]);
		if ($db->num_rows($result)) {
			return true;
		}
	...
	public function isEditableBySharing()
	{
		...
		$ownerResult = $db->pquery('SELECT owner FROM vtiger_report WHERE reportid = ?', [$this->getId()]);
		...
		if ($currentUserId == $reportOnwer) {
			...
			$result = $db->pquery($query, [$reportId, $reportId, $reportId, $reportId]);
			if ($db->num_rows($result)) {
				return false;
			} else {
				return true;
