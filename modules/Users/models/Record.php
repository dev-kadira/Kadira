...
class Users_Record_Model extends Vtiger_Record_Model
{
	 ...
	 */
	public function addDashboardTabs()
	{
		$db = PearDatabase::getInstance();
		$tabs = ['1'=>['name'=>'My Dashboard', 'app'=>'', 'module' =>'']];
		$isDefault = 1;
		foreach ($tabs as $seq => $tabName) {
			...
			$db->pquery(
				'INSERT INTO vtiger_dashboard_tabs(tabname,userid,isdefault,sequence,appname,modulename) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE tabname=?,userid=?,appname=?,modulename=?',
				[$tabName, $this->getId(), $isDefault, $seq, $appName, $moduleName, $tabName, $this->getId(), $appName, $moduleName]
			);
		}
	}
	...
	public function saveTagCloud()
	{
		$db = PearDatabase::getInstance();
		$db->pquery(
			"UPDATE vtiger_homestuff SET visible = ? WHERE userid=? AND stufftype='Tag Cloud'",
	...
	{
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT id, reports_to_id FROM vtiger_users where status = ?', ['Active']);
		...
		return array_diff($subUsers, [$forUserId]);
	}
