...
class Appointment
{
	...
	public static function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
	{
		...
		$startDate = new DateTimeField($from_datetime->year . '-' .
			$from_datetime->z_month . '-' . $from_datetime->z_day . " {$h}:{$m}");
		...
		$params = [$startDate->getDBInsertDateTimeValue(), $endDate->getDBInsertDateTimeValue()];
	...
	public function readResult($act_array, $view)
	{
		...
		if (! is_admin($current_user)) {
			if ($act_array['smownerid'] != 0 && $act_array['smownerid'] != $current_user->id && $act_array['visibility'] == 'Public') {
				...
				$row = $adb->pquery($que, [$current_user->id, $act_array['smownerid']]);
...
function getRoleBasesdPickList($fldname, $exist_val)
{
	...
	if ($is_Admin == 'off' && $fldname != '') {
		...
		$res = $adb->pquery($sql, [decode_html($exist_val)]);
		...
		if ($picklistvalueid != null) {
			...
			$res_val = $adb->pquery($pick_query, [$roleids]);
