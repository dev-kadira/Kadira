...
class CustomerPortal_SaveRecord extends CustomerPortal_FetchRecord
{
	...
	public function process(CustomerPortal_API_Request $request)
	{
		...
		if ($current_user) {
			...
			if (in_array($module, ['HelpDesk', 'Documents', 'Assets', 'Quotes', 'Contacts', 'Accounts'])) {
				$recordId = $request->get('recordId');
				if (! empty($recordId)) {
				...
				try {
					if (vtws_recordExists($recordId)) {
						...
						if ($module == 'HelpDesk' && ! empty($values['serviceid'])) {
							...
							$contact->save_related_module('HelpDesk', $crmId[1], 'ServiceContracts', [$ids[1]]);
						}

						if ($module == 'Documents') {
							$contact = new Contacts();
							$contact->save_related_module('Contacts', $this->getActiveCustomer()->id, 'Documents', [$recordId]);
