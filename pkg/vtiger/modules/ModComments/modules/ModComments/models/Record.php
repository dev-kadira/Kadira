...
class ModComments_Record_Model extends Vtiger_Record_Model
{
		...
		if (! empty($fileName)) {
			...
			if (function_exists('mb_strlen')) {
				if (mb_strlen(decode_html($temp_val)) > $listview_max_textlength) {
					$temp_val = mb_substr(preg_replace(
						'/(<\\/?)(\\w+)([^>]*>)/i',
						'',
						decode_html($field_val)
					), 0, $listview_max_textlength, $default_charset) . '...';
				}
	...
	public static function getInstanceById($record, $module=null)
	{
		...
		$query = 'SELECT vtiger_modcomments.*, vtiger_crmentity.smownerid,
	...
	public function getChildCommentsCount()
	{
		...
		$result = $db->pquery($query, [$this->getId(), $parentRecordId]);
		if ($db->num_rows($result)) {
	...
	public function getCommentedByName()
	{
		...
		if ($customer && ! empty($customer)) {
			...
			return $label;
		} elseif (($fromMailConverter || $fromMailroom) && $customerEmail && ! empty($customerEmail)) {
	 ...
	 * @return ModComments_Record_Model(s)
	 */
	public static function getAllComments($parentId)
	{
		$db = PearDatabase::getInstance();
		$focus = CRMEntity::getInstance('ModComments');
		$query = $focus->get_comments();
	...
	public function uploadAndSaveFile($emailId, $attachmentId)
	{
		$db = PearDatabase::getInstance();
		$db->pquery('INSERT INTO vtiger_seattachmentsrel(crmid,attachmentsid) VALUES(?,?)', [$emailId, $attachmentId]);
	}
