...
class QueryGenerator
{
	...
	public function getWhereClause()
	{
		...
		foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			$fieldSql = '(';
			$fieldGlue = '';
			$valueSqlList = $this->getConditionValue(
				$conditionInfo['value'],
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
					...
					if ($fieldName == 'created_user_id') {
						$concatSql = getSqlForNameInDisplayFormat(['first_name'=>"vtiger_users${fieldName}.first_name", 'last_name'=>"vtiger_users${fieldName}.last_name"], 'Users');
						$fieldSql .= "${fieldGlue} (trim(${concatSql}) ${valueSql})";
					} else {
						$concatSql = getSqlForNameInDisplayFormat(['first_name'=>'vtiger_users.first_name', 'last_name'=>'vtiger_users.last_name'], 'Users');
				...
				} elseif (Vtiger_Functions::isUserSpecificFieldTable($field->getTableName(), getTabModuleName($field->getTabId())) && $fieldName == 'starred'
		...
		if ($this->referenceModuleField) {
			foreach ($this->referenceModuleField as $index=>$conditionInfo) {
				if (empty($conditionInfo['SQLOperator'])) {
					continue;
				}
				$handler = vtws_getModuleHandlerFromName($conditionInfo['relatedModule'], $current_user);
				$meta = $handler->getMeta();
	...
	public function addRelatedModuleCondition($relatedModule, $column, $value, $SQLOperator)
	{
		$conditionNumber = $this->conditionInstanceCount++;
		$this->groupInfo .= "${conditionNumber} ";
		$this->manyToManyRelatedModuleConditions[$conditionNumber] = ['relatedModule'=> $relatedModule, 'column'=>$column, 'value'=>$value, 'SQLOperator'=>$SQLOperator];
	}
	...
	public function addReferenceModuleFieldCondition($relatedModule, $referenceField, $fieldName, $value, $SQLOperator, $glue = null)
	{
		...
		$this->groupInfo .= "${conditionNumber} ";
		$this->referenceModuleField[$conditionNumber] = ['relatedModule'=> $relatedModule, 'referenceField'=> $referenceField, 'fieldName'=>$fieldName, 'value'=>$value,
	...
	public function getDashBoardConditionList()
	{
		...
		if (! empty($dateClosedStart) && ! empty($dateClosedEnd)) {
			$conditionList[] = ['fieldname'=> 'closingdate', 'value'=>$dateClosedStart,
				'operator'                    => 'h'];
			$conditionList[] = ['fieldname'=> 'closingdate', 'value'=>$dateClosedEnd,
				'operator'                    => 'm'];
		}
		if (! empty($salesStage)) {
			if ($salesStage == 'Other') {
				$conditionList[] = ['fieldname'=> 'sales_stage', 'value'=>'Closed Won',
					'operator'                    => 'n'];
				$conditionList[] = ['fieldname'=> 'sales_stage', 'value'=>'Closed Lost',
					'operator'                    => 'n'];
			} else {
				$conditionList[] = ['fieldname'=> 'sales_stage', 'value'=> $salesStage,
					'operator'                    => 'e'];
			}
		}
		if (! empty($leadSource)) {
			$conditionList[] = ['fieldname'=> 'leadsource', 'value'=>$leadSource,
				'operator'                    => 'e'];
		}
		if (! empty($dateClosed)) {
			$conditionList[] = ['fieldname'=> 'closingdate', 'value'=>$dateClosed,
				'operator'                    => 'h'];
		}
		if (! empty($owner)) {
			$conditionList[] = ['fieldname'=> 'assigned_user_id', 'value'=>$owner,
				'operator'                    => 'e'];
		}
		$relatedConditionList = [];
		if (! empty($campaignId)) {
			$relatedConditionList[] = ['relatedModule'=>'Campaigns', 'conditionModule'=> 'Campaigns', 'finalValue'=>$campaignId, 'SQLOperator'=>'='];
		}
		if (! empty($quoteId)) {
			$relatedConditionList[] = ['relatedModule'=>'Quotes', 'conditionModule'=> 'Quotes', 'finalValue'=>$quoteId, 'SQLOperator'=>'='];
		}
		if (! empty($invoiceId)) {
			$relatedConditionList[] = ['relatedModule'=>'Invoice', 'conditionModule'=> 'Invoice', 'finalValue'=>$invoiceId, 'SQLOperator'=>'='];
		}
		if (! empty($purchaseOrderId)) {
			$relatedConditionList[] = ['relatedModule'=>'PurchaseOrder', 'conditionModule'=> 'PurchaseOrder', 'finalValue'=>$purchaseOrderId, 'SQLOperator'=>'='];
		}

		return ['conditions'=>$conditionList, 'relatedConditions'=>$relatedConditionList];
	}
	...
	protected function getConditionValue($value, $operator, $field)
	{
		...
		if (is_string($value) && $this->ignoreComma == false) {
			...
			if (in_array($operator, ['s', 'ew', 'c', 'k']) || in_array($field->getFieldDataType(), $commaSeparatedFieldTypes) || ($field->getFieldName() == 'salutationtype') || ($field->getFieldDataType() == 'reference' && in_array('Users', $field->getReferenceList()))) {
	...
	protected function getConditionalArray($fieldname, $value, $operator)
	{
		...
		return ['name'=>$fieldname, 'value'=>$value, 'operator'=>$operator];
	}
}
