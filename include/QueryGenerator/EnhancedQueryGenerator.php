...
class EnhancedQueryGenerator extends QueryGenerator
{
     ...
     * addTableIndexList
     *
     * @param  mixed $tablesList
     * @return void
     */
    public function addTableIndexList($tablesList)
    {
        if ($tablesList) {
     ...
     * @return void
     */
    public function getTableIndexList()
    {
        ...
        foreach ($advFilterList as $groupindex => $groupcolumns) {
            ...
            if (count($filtercolumns) > 0) {
                ...
                foreach ($filtercolumns as $index => $filter) {
                    ...
                    } elseif (($nameComponents[4] == 'D' || $nameComponents[4] == 'DT') && in_array($filter['comparator'], $specialDateTimeConditions)) {
        ...
        foreach ($this->conditionals as $index => $conditionInfo) {
            ...
            foreach ($valueSqlList as $valueSql) {
                if (in_array($baseFieldName, $this->referenceFieldList)) {
                    ...
                    } elseif (in_array('DocumentFolders', $moduleList)) {
                        $columnSql = "vtiger_attachmentsfolder".$fieldName.".foldername";
                    } elseif (in_array('Currency', $moduleList)) {
                    ...
                    if ($conditionInfo['operator'] == 'y' || ($conditionInfo['operator'] == 'e' && $valueSql == "= ''")) {
                        ...
                        $fieldGlue = ' OR';
                    } elseif ($conditionInfo['operator'] == 'k' || $conditionInfo['operator'] == 'n') {
                ...
                } elseif (($baseModule == 'Events' || $baseModule == 'Calendar') && ($field->getColumnName() == 'status' || $field->getColumnName() == 'eventstatus')) {
                ...
                } elseif (Vtiger_Functions::isUserSpecificFieldTable($field->getTableName(), getTabModuleName($field->getTabId())) && $fieldName == "starred" && $conditionInfo['value'] != 1) {
    ...
    public function getOrderByColumn($fieldName)
    {
        ...
        } elseif (($orderByFieldModel->getFieldName() == 'taskstatus' || $orderByFieldModel->getFieldName() == 'eventstatus') && $this->module == 'Calendar') {
     ...
     */
    public static function getSpecialDateConditionValue($comparator, $value, $type, $queryGenerator = false)
    {
        switch ($comparator) {
            ...
            case 'daysago':
                $startDate = date('Y-m-d', strtotime('-' . $value . ' days'));
                $endDate = date('Y-m-d', strtotime('-1 days'));
                if ($type == 'DT' || $type == 'D') {
                    return array('comparator' => 'bw', 'date' => array($startDate, $endDate));
                }
                return array('comparator' => 'e', 'date' => $startDate);
     ...
     */
    public static function getSpecialDateConditionQuery($comparator, $date)
    {
        switch ($comparator) {
