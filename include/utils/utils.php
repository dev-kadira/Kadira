...
function getActionname($actionid)
{
	...
	if (file_exists('tabdata.php') && (filesize('tabdata.php') != 0)) {
		...
		$result = $adb->pquery($query, [$actionid]);
...
function getRecordOwnerId($record)
{
	...
	if ($ownerId === false) {
		...
		$result = $adb->pquery($query, [$record]);
	...
	if ($ownerId) {
		...
		if ($count === false) {
			$sql_result = $adb->pquery('SELECT 1 FROM vtiger_users WHERE id = ?', [$ownerId]);
...
function insertProfile2field($profileid)
{
	...
	for ($i = 0; $i < $num_rows; $i++) {
		...
		$params = [$profileid, $tab_id, $field_id, 0, 0];
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from vtiger_field 
...
function transferUserCurrency($old_cur, $new_cur)
{
	...
	$adb->pquery($sql, [$new_cur, $old_cur]);
...
function transferProductCurrency($old_cur, $new_cur)
{
	...
	$prod_res = $adb->pquery('select productid from vtiger_products where currency_id = ?', [$old_cur]);
	...
	if (! empty($prod_ids) && (count($prod_ids) > 0)) {
		...
		for ($i = 0;$i < count($prod_ids);$i++) {
			...
			$params = [$new_cur, $unit_price, $product_id];
...
function transferPriceBookCurrency($old_cur, $new_cur)
{
	...
	$pb_res = $adb->pquery('select pricebookid from vtiger_pricebook where currency_id = ?', [$old_cur]);
...
function transferServicesCurrency($old_cur, $new_cur)
{
	...
	$ser_res = $adb->pquery('SELECT serviceid FROM vtiger_service WHERE currency_id = ?', [$old_cur]);
	...
	if (! empty($ser_ids) && (count($ser_ids) > 0)) {
		...
		for ($i = 0; $i < count($ser_ids); $i++) {
			...
			$params = [$new_cur, $unit_price, $service_id];
			$adb->pquery($query, $params);
		}
	}
...
function getCallerInfo($number)
{
	...
	foreach ($name as $module) {
		...
		if ($adb->num_rows($result) > 0) {
			...
			$data = ['name'=>$callerName, 'module'=>$module, 'id'=>$callerID];
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo)
{
	...
	$result = $adb->pquery($sql, [$userExtension]);
	...
	if ($status == 'outgoing') {
		...
		$sql = 'select * from vtiger_asteriskextensions where asterisk_extension=?';
		$result = $adb->pquery($sql, [$callfrom]);
		if ($adb->num_rows($result) > 0) {
			$userid = $adb->query_result($result, 0, 'userid');
	...
	$adb->pquery($query, [$crmID, $userID, $userID, 0, 'PBXManager', '', $timeOfCall, $timeOfCall, null, null, 0, 1, 0, $callerName]);

	$sql = 'insert into vtiger_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)';
	$params = [$crmID, $callerName, $receiver, $timeOfCall, $status];
...
function getRelationTables($module, $secmodule)
{
	...
	$query = 'SELECT vtiger_field.tabid AS tabid,vtiger_field.tablename AS tablename, vtiger_field.columnname AS columnname 
				FROM vtiger_field INNER JOIN vtiger_fieldmodulerel ON vtiger_fieldmodulerel.fieldid = vtiger_field.fieldid 
				WHERE (vtiger_fieldmodulerel.module=? AND vtiger_fieldmodulerel.relmodule=?) OR (vtiger_fieldmodulerel.module=? 
				AND vtiger_fieldmodulerel.relmodule=?) ORDER BY vtiger_fieldmodulerel.sequence ASC';

	$ui10_query = $adb->pquery($query, [$module, $secmodule, $secmodule, $module]);
...
{
	$sql = 'select distinct vtiger_field.tabid, name from vtiger_field
	...
	$it = new SqlResultIterator($adb, $adb->pquery($sql, []));
	$modules = [];
...
function getTabInfo($tabId)
{
	...
	$tabInfoResult = $adb->pquery('SELECT prefname, prefvalue FROM vtiger_tab_info WHERE tabid=?', [$tabId]);
	$tabInfo = [];
...
function getBlockName($blockid)
{
	...
	if (! empty($blockid) && $blockname === false) {
		$block_res = $adb->pquery('SELECT blocklabel FROM vtiger_blocks WHERE blockid = ?', [$blockid]);
...
{
	return ['Invoice', 'Quotes', 'PurchaseOrder', 'SalesOrder'];
}
...
function getRecordOwnerReportsToId($record)
{
	...
	if ($ownerArr['Users']) {
		...
		$result = $adb->pquery($query, [$ownerArr['Users']]);
		if ($adb->num_rows($result) > 0) {
...
function getDuplicatesPreventionMessage($moduleName, $duplicateRecordsList)
{
	...
	if ($duplicateRecordsCount === 6) {
		...
		foreach ($uniqueFields as $fieldModel) {
			...
			$comparator = 'e';
			if (in_array($fieldDataType, ['date', 'datetime'])) {
				$comparator = 'bw';
