...
function vtws_convertlead($entityvalues, $user)
{
	...
	$results = $adb->pquery($getQuote, [$leadId[1]]);
				...
				if ($create) {
					...
					if (($leadHasImage) && ((($entityName == 'Contacts') || ($entityName == 'Accounts' && ! $entityvalues['entities']['Contacts']['create'])))) {
						$imageName = $adb->query_result($adb->pquery('SELECT name FROM vtiger_attachments 
							WHERE attachmentsid = ?', [$imageAttachmentId]), 0, 'name');
					...
					if ($leadHasImage && in_array($entityName, ['Accounts', 'Contacts'])) {
						$idComponents = explode('x', $entityIds[$entityName]);
						$crmId = $idComponents[1];
						$adb->pquery('UPDATE vtiger_seattachmentsrel SET crmid = ? WHERE attachmentsid = ?', [$crmId, $imageAttachmentId]);
						$adb->pquery('UPDATE vtiger_crmentity SET setype = ? WHERE crmid = ?', [$entityName . ' Image', $imageAttachmentId]);
					}
				}
	...
	try {
		...
		if ($quoteIds) {
			...
			$adb->pquery($queryUpdate, [$contactId, $potentialId, $quoteIds]);

			if ($accountId) {
				$queryUpdate = 'UPDATE vtiger_quotes SET accountid=? WHERE quoteid IN(' . generateQuestionMarks($quoteIds) . ')';
				$adb->pquery($queryUpdate, [$accountId, $quoteIds]);
			}
		}
