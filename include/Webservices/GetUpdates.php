...
function vtws_sync($mtime, $elementType, $syncType, $user)
{
	...
	if ($applicationSync && ! is_admin($user)) {
		throw new WebServiceException(
			...
			'Only admin users can perform application sync'
		);
	}
		...
		while ($arre = $adb->fetchByAssoc($result)) {
			...
			if (vtws_isRecordDeleted($arre, $deleteColumnNames, $deleteFieldValues)) {
				...
				try {
					...
					throw new WebServiceException(
						WebServiceErrorCode::$INTERNALERROR,
	...
	$q = "SELECT crmid FROM {$baseCRMTable} WHERE modifiedtime>? 
		AND setype IN(" . generateQuestionMarks($accessableModules) . ')';
...
/**
 * vtws_isRecordDeleted
 *
 * @param  mixed $recordDetails
 * @param  mixed $deleteColumnDetails
...
function getSyncQueryBaseTable($elementType)
{
	if ($elementType != 'Calendar' && $elementType != 'Events') {
		...
		return "vtiger_crmentity INNER JOIN vtiger_activity 
			ON (vtiger_crmentity.crmid = vtiger_activity.activityid and {$activityCondition})";
	}
}

/**
 * getCalendarTypeCondition
 *
 * @param  mixed $elementType
 * @return String
 */
