...
class VtigerCRMActorMeta extends EntityMeta
{
	 ...
	 * @return void
	 */
	public function VtigerCRMActorMeta($tableName, $webserviceObject, $adb, $user)
	{
	...
	public function getEntityTableIndexList()
	{
		if ($this->tableIndexList === null) {
			$this->tableIndexList = [$this->baseTable=>$this->getIdColumn()];
		}
	...
	public function exists($recordId)
	{
		...
		$result = $this->pearDB->pquery($sql, [$recordId]);
	...
	public function getNameFields()
	{
		...
		$result = $this->pearDB->pquery($query, [$this->objectId]);
		$fieldNames = '';
		if ($result) {
	...
	public function getName($webserviceId)
	{
		...
		$nameList = vtws_getActorEntityNameById($entityId, [$id]);

		return $nameList[$id];
	}

	/**
	 * getEntityAccessControlQuery
	 *
	 * @return void
	 */
