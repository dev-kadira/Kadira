...
class DataTransform
{
	...
	public static function sanitizeDataWithColumn($row, $meta)
	{
		...
		return DataTransform::sanitizeData($newRow, $meta, true);
	}
	...
	public function sanitizeData($newRow, $meta, $t = null)
	{
	...
	public function filterAllColumns($row, $meta)
	{
	...
	public function sanitizeFields($row, $meta)
	{
	...
	public function sanitizeReferences($row, $meta)
	{
		global $adb,$log;
		$references = $meta->getReferenceFieldDetails();
		foreach ($references as $field=>$typeList) {
	...
	public function sanitizeOwnerFields($row, $meta, $t = null)
	{
		global $adb;
		$ownerFields = $meta->getOwnerFields();
		foreach ($ownerFields as $index=>$field) {
	 ...
	 * @return <array>
	 */
	public static function sanitizeFileFieldsForIds($row, $meta)
	{
		$moduleFields = $meta->getModuleFields();
		$supportedUITypes = [61, 69, 28]; //file and image uitypes
		$attachmentIds = [];
