...
/**
 * vtws_convertPotential
 *
 * @param  mixed $entityvalues
 * @param  mixed $user
 * @return Array
 */
function vtws_convertPotential($entityvalues, $user)
{
	...
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			$entityObjectValues = vtws_populateConvertPotentialEntities(
				$entityvalue,
				$entityObjectValues,
				$entityHandler,
				$potentialHandler,
			...
			try {
				...
				throw new WebServiceException(
					WebServiceErrorCode::$FAILED_TO_CREATE,
...
/**
 * vtws_convertPotentialTransferHandler
 *
 * @param  mixed $potentialIdComponents
...
function vtws_convertPotentialTransferHandler($potentialIdComponents, $entityIds, $entityvalues)
{
	try {
		...
		vtws_transferPotentialRelatedRecords(
			$potentialIdComponents[1],
			$entityidComponents[1],
			$entityvalues['transferRelatedRecordsTo']
		);
	} catch (Exception $e) {
		return false;
	}
...
/**
 * vtws_updateConvertPotentialStatus
 *
 * @param  mixed $entityIds
...
function vtws_updateConvertPotentialStatus($entityIds, $potentialId, $user)
{
	...
	if ($entityIds['Project'] != '' || $entityIds['Contacts'] != '') {
		...
		if ($result === false) {
			throw new WebServiceException(
				WebServiceErrorCode::$FAILED_TO_MARK_POTENTIAL_CONVERTED,
				'Failed mark potential converted'
			);
		}
